...
public class ANN1 
{
	#region Variables
		...
		public double alpha;
		List<Layer1> layers = new List<Layer1>();
	#endregion
	...
	public ANN1(int nI, int nO, int nH, int nPH, double a)
	{
		...
		if (numHidden > 0)
		{
			layers.Add(new Layer1(numNPerHidden, numInputs));

			for (int i = 0; i < numHidden - 1; i++)
			{
				layers.Add(new Layer1(numNPerHidden, numNPerHidden));
			}

			layers.Add(new Layer1(numOutputs, numNPerHidden));
		}
		else
		{
			layers.Add(new Layer1(numOutputs, numInputs));
		}
	}
	...
	{
		string weightStr = "";
		foreach (Layer1 l in layers)
		{
			foreach (Neuron1 n in l.neurons)
			{
