...
public class ANN
{
	...
	public List<double> Go(List<double> inputValues, List<double> desiredOutput)
	{
		...
		for (int i = 0; i < numHidden + 1; i++)
		{
			...
			for (int j = 0; j < layers[i].numNeurons; j++)
			{
				...
				//if i pointing the last layer which is the output layer run the activation function 
				// with this way you can mix the activation funcitons. 
				if (i == numHidden)
	...
	{
		//you can only output values between 0 and 1 
		double k = (double)System.Math.Exp(value);
		return k / (1.0f + k);
	}
	double TanH(double value)
    {
		// if you want to output negative values you should use TanH. 
		/* For example if we need the player to move in positive and negative direction we can't do it with sigmoid. */
		return (2 * (Sigmoid(2*value)) - 1);
    }
	double ReLu (double value)
    {
		if (value > 0) return value; // the leaniar part in the positive values. 
		else return 0; // the flat part in the negative input values. 
    }
